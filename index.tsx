    ...
    <ul className="pl-0 mr-12">
      {search.pages('post').map((post) => (
        <li key={post.title} className="list-none pb-5 mb-8 lg:mb-5 border-b border-solid border-slate-700">
          <a className="py-5 dark:text-sky-400 lg:text-lg" href={post.url}>{post.title}</a> <span className="hidden lg:inline ml-3 text-sm">{post.readingInfo.minutes} min read</span>
